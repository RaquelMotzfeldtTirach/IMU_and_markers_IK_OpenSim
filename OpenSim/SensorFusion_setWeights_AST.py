...
def main(subject_ID=None, trial_ID=None, subject_mass=None, subject_height=None, subject_age=None, subject_sex=None, lag=None):
    ...
    results_folder_rmse = f'analytics/AST_scaling/subject_{subject_ID}/{trial_ID}/rmse/'
    ...
    # Make a folder for the vicon results
    results_folder_vicon = f'analytics/AST_scaling/subject_{subject_ID}/{trial_ID}/ground_truth/'
    if not os.path.exists(results_folder_vicon):
        os.makedirs(results_folder_vicon)
    results_file_gt = f'analytics/AST_scaling/subject_{subject_ID}/{trial_ID}/ground_truth/inverse_kinematics_results_subject_{subject_ID}_trial_{trial_ID}.mot'

    # Fetch the already scaled model
    scaled_model_path = f'OpenSim/Models/Rajagopal/AST_scaled_models/Scaled_rajagopal2015_vicon_subject{subject_ID}.osim'
  
    # Compute vicon IK results (ground truth)
    results_file_vicon = vicon_ik(subject_ID, trial_ID, constraint_var, subject_mass, subject_height, subject_age, subject_sex, scaled_model_path=scaled_model_path)
    
    # Copy results to the analytics folder for record keeping
    shutil.copy(results_file_vicon, results_file_gt)
    ...
    # Scaled model path 
    scaled_model_path = f'OpenSim/Models/Rajagopal/AST_scaled_models/Scaled_rajagopal2015_webcam_subject{subject_ID}.osim'
    ...
    if os.path.exists(trial_data_filepath_webcam):
        ...
        for sensor_count, best_row in best_rows_webcam:
            ...
            stereocamera_weights_run = [0.0] * 15
            fusion_run = sensor_fusion_initialization(webcam_weights_run, best_orientation_weights, stereocamera_weights_run,
                                                        constraint_var, subject_ID, trial_ID, subject_mass, subject_height, subject_age, subject_sex, scaled_model_path=scaled_model_path)
    ...
    # Scaled model path
    scaled_model_path = f'OpenSim/Models/Rajagopal/AST_scaled_models/Scaled_rajagopal2015_stereocamera_subject{subject_ID}.osim'
