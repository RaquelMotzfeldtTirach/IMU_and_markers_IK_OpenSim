...
import argparse
import glob
from SensorFusion_setWeights_AST import main as ik_set_weights_ast
...
def main(ID): 
    ...
    # Then run rmse calculation for the rest of trials
    for trial in trials:
        if trial != "static":
            ik_set_weights_ast(subject_ID=subject_id, trial_ID=trial, subject_mass=info['weight'], subject_age=info['age'], subject_height=info['height'], subject_sex=info['sex'], lag=lags[trial])
